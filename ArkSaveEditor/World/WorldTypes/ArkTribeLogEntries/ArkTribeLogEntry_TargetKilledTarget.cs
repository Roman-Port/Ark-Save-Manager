...
namespace ArkSaveEditor.World.WorldTypes.ArkTribeLogEntries
{
    public class ArkTribeLogEntry_TargetKilledTarget : ArkTribeLogEntry
    {
        ...
        public ArkTribeLogEntry_TargetKilledTarget(string content, List<ArkPlayerProfile> playerProfiles, List<ArkDinosaur> globalDinos, int tribeId)
        {
            //Use the regex to get groups
            GroupCollection gc = GetRegexGroups(content, ArkTribeLogEntry.REGEX_TARGETKILLEDTARGET);

            //Grab targets
            victim = TryFindDinoOrPlayerProfile(gc[2], gc[3], gc[4], playerProfiles, globalDinos, tribeId, false);
            killer = TryFindDinoOrPlayerProfile(gc[9], gc[10], gc[11], playerProfiles, globalDinos, tribeId, gc[8].Success); //If group 8 is not empty, this is a wild dino
